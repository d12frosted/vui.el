...
- Pixel measurement is more expensive per call. Results are memoized
  ...
  default.
- The cache keys on string content only, so a bold header and a plain
  cell with the same text share one measurement. Fine when faces keep
  the same metrics, off when a face changes the font or its size. In a
  proportional font that shows up as a bold table header a few pixels
