...
shallow-equal (=equal= on each value, except that strings must also
match in text properties - a prop whose face changed counts as changed)
and state is unchanged, like =React.memo=. These two components behave
the same:
