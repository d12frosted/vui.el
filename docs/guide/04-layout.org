...
- Bordered tables in a proportional font depend on the font's
  box-drawing glyphs having consistent widths, and they often do not:
  Helvetica on macOS draws =─= and =│= at 12px but falls back to Arial
  for the junctions (=┼=, =├=, =┬=) at 9px, and no count of whole fill
  glyphs can make such a border row match the data rows. Data rows,
  boxes and flex rows align exactly; if a border drifts under
  =variable-pitch-mode=, use =:border nil=.
