...
|-------------+----------------------------------------------|
| =:id=       | Optional identifier for this boundary        |
| =:fallback= | Function =(lambda (error) vnode)=            |
| =:on-error= | Optional callback, called once per caught error |
| =:children= | Protected content                            |

=:id= may be omitted: the boundary's identity is then derived from its
position in the rendered tree, which is stable across re-renders as long
as the tree shape around it does not change. Provide an explicit =:id=
when the boundary moves around the tree, or when you want to reset it by
name with =vui-reset-error-boundary=.

Error state is scoped to the mounted tree (or to the buffer for static
=vui-render= trees): two apps using the same =:id= do not interfere, and
a fresh =vui-mount= always starts with a clean slate.
...
#+begin_src elisp
(vui-defcomponent error-recovery ()
  :render
  (vui-error-boundary
   :id "recoverable"
   :fallback (lambda (err)
...
Calling =vui-reset-error-boundary= without an id resets every boundary
in the current tree.
...
#+begin_src elisp
(vui-defcomponent button-that-errors ()
  :render
  (vui-button "Click me"
              :on-click (lambda ()
...
#+begin_src elisp
(vui-defcomponent safe-data-display ()
  :state ((data nil)
          (error nil))
