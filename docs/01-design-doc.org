...
|  Handles cursor preservation via widget path tracking           |
+-----------------------------------------------------------------+
                              |
                              v
+-----------------------------------------------------------------+
...
+--------------+
     ...
     v
+--------------+
| Save cursor  |  Track cursor position via widget path + offset
+--------------+
     |
     v
+--------------+
...
+--------------+
     ...
     v
+--------------+
| Restore      |  Restore cursor to widget at same path
+--------------+
     |
     v
+--------------+
...
*Decision 5: Text buttons for clickables, widget.el only for fields*
- =vui-button=, =vui-checkbox= and =vui-select= render as =button.el= text
  buttons (marker-free and cheap; see #107)
- =vui-field= wraps =editable-field= widget (widget.el has the only
  battle-tested in-buffer text editing)
- TAB/S-TAB navigation is vui's own (=vui-forward= / =vui-backward=),
  stepping across both text buttons and fields; field editing comes free
  from =widget.el=
- We provide: component model, state management, layout, reconciliation,
  unified navigation and cursor tracking over the mixed set
- (Historically all four wrapped =widget.el= widgets; buttons moved to
  =button.el= because each widget left markers that made rendering O(n^2))
...
- Cursor preserved by tracking widget path (vnode tree position) before/after
...
|                  | Cursor preservation (widget path tracking) | ✓      |
