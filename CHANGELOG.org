...
- =pixel= width mode now survives =text-scale-mode= and measures correctly
  ...
  =variable-pitch-mode=. Known limits: a proportional font whose box-drawing
  junctions come from a different fallback than its rules (Helvetica draws
  =─= at 12px and =┼= at 9px) cannot have a glyph-built border row match the
  data rows, so use =:border nil= there. Byte-identical in char mode (#121).
