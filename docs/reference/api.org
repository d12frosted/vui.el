...
text so the blocks sit side by side; buttons keep working there, but
widget fields and per-component regions do not survive text
composition. Give stateful components inside a wrapped flex a =:key=.
...
a multi-line cell are composed as text (buttons work; widget fields
and per-component regions do not survive composition). Give stateful
components in a grid a =:key=.
