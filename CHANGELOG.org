...
- Mounted buffers reflow on window resize by default: =vui-mount= and
  ...
  respond to resizes with no setup. The hook only runs on actual size
  changes of the buffer's windows and coalesces through the normal
  deferred scheduling. Set =vui-rerender-on-resize-default= to nil for
  the previous opt-in behavior; plain =vui-render= buffers still need
  the explicit call (#134).
