...
- =vui-width-mode=: choose how layout measures and pads text. The default,
  ...
  =after-setting-font-hook=; the cache keys on string content only, so two
  faces with different font metrics but identical text share an entry (a
  documented approximation). Measured on the table benchmarks pixel mode
...
- =pixel= width mode now survives =text-scale-mode= and measures correctly
  ...
  data rows, so use =:border nil= there; and a bold header in a
  proportional font is still measured without its face (see the cache note
  in the layout guide). Byte-identical in char mode (#121).
