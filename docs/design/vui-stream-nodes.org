...
Implemented: =vui-stream-open=, =vui-stream-append-to=, =vui-stream-update=,
=vui-stream-finalize=, and the out-of-order =vui-stream-before= /
=vui-stream-after= / =vui-stream-remove=, with differential tests
(=test/vui-stream-node-test.el=) - out-of-order edits stay byte-identical to
a wholesale render, the model is kept in sync so a full re-render re-emits
the streamed content, =finalize= releases the node's markers, and
insert/remove keep separators single (emptying the stream re-lays, the
mirror of the empty -> non-empty transition). The risky question ("does a
ref-based node API throw away vui-stream's O(1) append?") is answered below
and borne out in the build: no, *if* finalized nodes drop their markers.
=vui-stream-open= also accepts a =vui-component= - a stateful inline ROW
addressable by ref, updated out of order with =vui-stream-update= (props
refresh, state preserved); such a node carries no markers of its own (the
row owns them), so =finalize= just stops tracking it and the row stays
interactive. Still to come: node-owned separators and the section tree.
...
- *Component nodes* (interactive rows: collapsible cards) are inline instances
  ...
  separate problem. As implemented, =vui-stream-open= of a component returns a
  node that wraps the inline row; =vui-stream-update= refreshes its props out
  of order (state preserved); =finalize= just stops tracking the node (the row
  keeps its own markers and stays interactive). =append-to= / =before= /
  =after= / =remove= are content-only for now and no-op on a component node.
  (Today's component-rows code already has the latent O(rows)-per-append cost;
  the node model makes it addressable.)
