...
- Measuring a mounted component now sees its current state. Layout
  ...
  actual reconciliation. Wired into =vui-flex='s child measurement;
  matching is exact for keyed children and for unkeyed children in
  measure order (a =vui-flex-item= wrapping a component earlier in the
  row can skew unkeyed index matching; give such children a =:key=).
  Groundwork for the responsive layout pass of #134, which measures
  every layout child this way (=vui--measure-block=).
...
- =vui-grid=: a responsive grid of equal-width tracks (#134). Cells
  ...
  =:row-spacing= inserts blank lines between rows. Identity follows
  =vui-flex= =:wrap=: single-line rows render in place, rows with
  multi-line cells are composed as text. Works in both width modes.
...
- =vui-flex= accepts =:wrap t=: children flow into rows in source order
  ...
  narrow. Identity is per row: a row whose children are all single-line
  ...
  text like a table cell (buttons work; widget fields and component
  regions do not survive composition). =:justify= is not applied under
  =:wrap=. Works in both =char= and =pixel= width modes.
