...
First increment implemented: =vui-stream-open=, =vui-stream-append-to=,
=vui-stream-update=, and =vui-stream-finalize= ship with differential tests
...
the streamed content, and =finalize= releases the node's markers. The risky
question ("does a ref-based node API throw away vui-stream's O(1) append?")
is answered below and borne out in the build: no, *if* finalized nodes drop
their markers. Still to come: out-of-order =before= / =after= / =remove=,
component nodes via =open=, node-owned separators, and the section tree.
