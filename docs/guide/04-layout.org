...
- Bordered tables in a proportional font: the font's box-drawing
  ...
  spacer is a gap of less than one glyph in the rule; a corner wider
  than the separator (ASCII =+= over =|=) overhangs the table by half
  the difference. In a monospace font the output is byte-identical to
  before.
