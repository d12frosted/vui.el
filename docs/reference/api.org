#+TITLE: API Reference
#+AUTHOR: vui.el Documentation
#+OPTIONS: toc:3 num:t H:4

Complete reference for all vui.el functions, macros, and variables.

* Component Definition

** defcomponent

#+begin_src elisp
(vui-defcomponent NAME (PROPS...) [DOCSTRING] BODY...)
#+end_src

Define a component named NAME.

*** Arguments

- NAME :: Symbol naming the component
- PROPS :: List of prop names the component accepts
- DOCSTRING :: Optional documentation string

*** Body Keywords

| Keyword          | Required | Description                                 |
|------------------+----------+---------------------------------------------|
| =:state=         | No       | =(VAR INITIAL)...= - local state variables  |
| =:on-mount=      | No       | Form run after first render                 |
| =:on-update=     | No       | Form run after re-render (not first)        |
| =:on-unmount=    | No       | Form run before removal                     |
| =:should-update= | No       | Return t to allow re-render, nil to skip    |
| =:memo=          | No       | =t= shorthand for a shallow-prop should-update |
| =:render=        | Yes      | The render expression (must return a vnode) |

=:memo t= is a shorthand for the most common =:should-update=: skip the
re-render while props are shallow-equal (=equal= on each value, except
that strings must also match in text properties) and state is unchanged,
like =React.memo=.  An explicit =:should-update= always
takes precedence over =:memo=.  Note that =:children= counts as a prop,
so a memo component wrapping nested content re-renders whenever its
parent does (the children vnodes are freshly built each render); =:memo=
pays off for leaf-ish components driven purely by their own props.

*** Available Variables in Body

In all forms:
- Props as named variables (e.g., =title= for =:title= prop)
- State vars as named variables
- =children= for nested content

In =:on-update= and =:should-update= additionally:
- =props= - Current props plist
- =state= - Current state plist
- =prev-props= - Previous props plist
- =prev-state= - Previous state plist

*** Example

#+begin_src elisp
(vui-defcomponent counter (initial-value)
  "A counter component with increment button."
  :state ((count initial-value))

  :on-mount
  (message "Counter mounted with %d" count)

  :on-update
  (unless (equal (plist-get prev-state :count)
                 (plist-get state :count))
    (message "Count changed to %d" count))

  :should-update
  (not (equal (plist-get prev-state :count)
              (plist-get state :count)))

  :on-unmount
  (message "Counter unmounted")

  :render
  (vui-fragment
   (vui-text (format "Count: %d" count))
   (vui-button "+" :on-click (lambda ()
                               (vui-set-state :count (1+ count))))))
#+end_src

* Mounting

** vui-mount

#+begin_src elisp
(vui-mount COMPONENT-VNODE &optional BUFFER-NAME) -> instance
#+end_src

Mount a component as root and render to a buffer.

If the buffer already has a mounted instance, it is unmounted first
(see =vui-unmount=), so the old tree's lifecycle cleanups run before the
new tree renders.

- COMPONENT-VNODE :: Created with =vui-component=
- BUFFER-NAME :: String (default ="*vui*"=)
- Returns :: The root instance

#+begin_src elisp
(vui-mount (vui-component 'my-app :title "Hello") "*my-app*")
#+end_src

** vui-mount-inline

#+begin_src elisp
(vui-mount-inline COMPONENT-VNODE &optional POSITION) -> instance
#+end_src

Mount a component inline at POSITION (default: point) in the current
buffer, without taking the buffer over.

Unlike =vui-mount=, the component renders into a managed region inside
the existing buffer content: the buffer's major mode is left untouched,
and a buffer can host any number of inline instances. State updates
rewrite only the instance's region.

The region is ephemeral UI, not document content: vui keeps its rewrites
out of the undo history, and manual edits inside the region (outside of
input fields) are overwritten by the next re-render. POSITION must not
fall strictly inside another inline instance's region.

Pass the returned instance to =vui-unmount= to dismiss the UI (teardown
also runs automatically when the buffer is killed), or drive it with
=vui-rerender= / =vui-update= / =vui-update-props=.

#+begin_src elisp
;; An ephemeral form that dismisses itself on submit
(let ((form nil))
  (setq form
        (vui-mount-inline
         (vui-component 'server-query-form
           :on-submit (lambda (params)
                        (vui-unmount form)
                        (run-query params))))))
#+end_src

** vui-inline-instance-at

#+begin_src elisp
(vui-inline-instance-at &optional POSITION) -> instance or nil
#+end_src

Return the inline instance whose region contains POSITION (default:
point) in the current buffer, or nil if there is none. Useful for
commands like "dismiss the form at point":

#+begin_src elisp
(when-let* ((form (vui-inline-instance-at)))
  (vui-unmount form))
#+end_src

** vui-unmount

#+begin_src elisp
(vui-unmount &optional BUFFER-OR-INSTANCE) -> instance or nil
#+end_src

Unmount a VUI instance, running its full teardown lifecycle.

BUFFER-OR-INSTANCE selects what to unmount:
- nil, a buffer, or a buffer name :: the instance mounted in that buffer
  (default: current buffer) via =vui-mount=. The buffer's content is
  erased, but the buffer itself is not killed.
- an instance returned by =vui-mount-inline= :: its managed region is
  removed from the host buffer; the rest of the buffer is untouched.
- an instance returned by =vui-mount= :: same as passing its buffer.

The teardown lifecycle runs for every component in the tree:
=:on-unmount= hooks, effect cleanup functions, cleanup functions returned
from =:on-mount=, and cancellation of pending async processes and deferred
renders.

After unmounting, async callbacks created via =vui-with-async-context= or
=vui-async-callback= that captured this tree become no-ops.

- Returns :: The unmounted instance, or nil if nothing was mounted

#+begin_src elisp
(vui-unmount "*my-app*")
(vui-unmount inline-form-instance)
#+end_src

** vui-get-instance

#+begin_src elisp
(vui-get-instance &optional BUFFER) -> instance or nil
#+end_src

Return the root component instance mounted in BUFFER.

- BUFFER :: Buffer object or buffer name (default: current buffer)
- Returns :: The root instance, or nil if BUFFER does not exist or has no
  mounted VUI instance

Use this together with =vui-rerender=, =vui-update=, and =vui-update-props=
to drive a mounted UI from outside, without storing the instance yourself:

#+begin_src elisp
(when-let* ((instance (vui-get-instance "*sidebar*")))
  (vui-update instance (list :note note)))
#+end_src

The instance returned is the root - the component passed to =vui-mount=. To
push data into a nested component, or into state rather than props, see
[[file:../guide/12-external-updates.org][External Updates]].

** vui-rerender

#+begin_src elisp
(vui-rerender INSTANCE) -> instance
#+end_src

Re-render an existing instance, preserving component state.

Triggers a re-render of the component tree rooted at INSTANCE. Component state
(including collapsed sections, internal state) is preserved through
reconciliation. Memoized values are also preserved and will only recompute if
their dependencies change.

Returns INSTANCE for chaining.

#+begin_src elisp
;; Re-render without changing anything (useful for forcing a redraw)
(vui-rerender instance)
#+end_src

** vui-update

#+begin_src elisp
(vui-update INSTANCE NEW-PROPS) -> instance
#+end_src

Update instance props, invalidate all memos, and re-render.

Use this when external data has changed and cached computations should be
discarded (e.g., after a file save, database update, or user-triggered refresh).

NEW-PROPS completely replaces the instance's current props, so pass every prop
the component needs, not only the changed ones. All memoized values in the
instance tree are invalidated, forcing recomputation on the next render.
Component state is preserved through reconciliation.

State is preserved, *not* re-seeded: a component whose =:state= is initialized
from a prop keeps the value it captured at mount, and new props for that prop
are ignored with no error. Read the prop directly in =:render=, or sync it in
=:on-update=. See [[file:../guide/12-external-updates.org][External Updates]].

Returns INSTANCE for chaining.

#+begin_src elisp
;; After save - data on disk changed, force recomputation
(vui-update instance (list :note (fetch-note)))
#+end_src

** vui-update-props

#+begin_src elisp
(vui-update-props INSTANCE NEW-PROPS) -> instance
#+end_src

Update instance props and re-render, preserving memos.

Use this for periodic or speculative refreshes where data may not have changed.
Memoized values are preserved and only recompute if their dependencies change,
avoiding unnecessary expensive work.

NEW-PROPS completely replaces the instance's current props, so pass every prop
the component needs, not only the changed ones. As with =vui-update=, state
seeded from a prop is preserved rather than re-seeded.

Returns INSTANCE for chaining.

#+begin_src elisp
;; Idle timer - data probably hasn't changed, let memos decide
(vui-update-props instance (list :note (fetch-note)))
#+end_src

** When to use which

| Function            | Props   | Memos       | Use case                                    |
|---------------------+---------+-------------+---------------------------------------------|
| =vui-rerender=      | Keep    | Preserved   | Force redraw, same props                    |
| =vui-update-props=  | Replace | Preserved   | Periodic refresh, data may not have changed |
| =vui-update=        | Replace | Invalidated | Data changed, force recomputation           |

As a rule of thumb:
- *Save hooks, manual refresh* → =vui-update= (data changed, recompute everything)
- *Idle timers, window focus* → =vui-update-props= (data probably unchanged, let memos skip work)
- *Internal triggers* → =vui-rerender= (just redraw with current props and state)

* Primitives

Interactive primitives (buttons, fields, checkboxes, selects) support =TAB= /
=S-TAB= keyboard navigation.

** vui-text

#+begin_src elisp
(vui-text CONTENT &rest PROPS) -> vnode
#+end_src

Create a text node.

| Property | Type   | Description             |
|----------+--------+-------------------------|
| =:face=  | symbol | Emacs face for styling  |
| =:key=   | any    | Reconciliation key      |

#+begin_src elisp
(vui-text "Hello" :face 'bold)
(vui-text (format "Count: %d" count))
#+end_src

** vui-newline

#+begin_src elisp
(vui-newline &optional KEY) -> vnode
#+end_src

Create a newline.

** vui-space

#+begin_src elisp
(vui-space &optional WIDTH KEY) -> vnode
#+end_src

Create WIDTH spaces (default 1).

** vui-fragment

#+begin_src elisp
(vui-fragment &rest CHILDREN) -> vnode
#+end_src

Group multiple vnodes without adding structure.

#+begin_src elisp
(vui-fragment
 (vui-text "Line 1")
 (vui-newline)
 (vui-text "Line 2"))
#+end_src

** vui-button

#+begin_src elisp
(vui-button LABEL &rest PROPS) -> vnode
#+end_src

Create a clickable button. By default, buttons render with brackets: =LABEL= becomes =[LABEL]=.

| Property         | Type        | Description                                |
|------------------+-------------+--------------------------------------------|
| =:on-click=      | function    | Click handler                              |
| =:face=          | symbol      | Button face                                |
| =:disabled=      | boolean     | Disable button                             |
| =:max-width=     | integer     | Max width including brackets (truncates)   |
| =:no-decoration= | boolean     | Render without brackets                    |
| =:help-echo=     | string/nil  | Tooltip (nil disables for performance)     |
| =:tab-order=     | integer     | Tab navigation order (-1 = skip)           |
| =:keymap=        | keymap      | Custom keymap active when on button        |
| =:key=           | any         | Reconciliation key                         |

#+begin_src elisp
(vui-button "Click me"
            :on-click (lambda () (message "Clicked!"))
            :face 'custom-button)
;; Renders: [Click me]

;; Truncated button
(vui-button "Very long label" :max-width 12)
;; Renders: [Very lo...]

;; Plain button (no brackets)
(vui-button "Click here" :no-decoration t)
;; Renders: Click here

;; Disable tooltip for performance (useful with many buttons)
(vui-button "Item" :help-echo nil)
#+end_src

** vui-field

#+begin_src elisp
(vui-field &key VALUE SIZE PLACEHOLDER ON-CHANGE ON-SUBMIT KEY FACE SECRET) -> vnode
#+end_src

Create a text input field. This is a simple string-based primitive. For typed
fields with parsing and validation, use =vui-typed-field= from =vui-components.el=.

| Property       | Type     | Description                     |
|----------------+----------+---------------------------------|
| =:value=       | string   | Initial content (default "")    |
| =:size=        | integer  | Width in characters             |
| =:placeholder= | string   | Hint text shown while empty     |
| =:on-change=   | function | Called with value on change     |
| =:on-submit=   | function | Called with value on RET        |
| =:key=         | any      | Reconciliation key / lookup key |
| =:face=        | symbol   | Text face                       |
| =:secret=      | boolean  | Hide input (for passwords)      |

#+begin_src elisp
(vui-field :value name
           :size 20
           :on-change (lambda (v) (vui-set-state :name v)))

;; Password field
(vui-field :value password
           :size 20
           :secret t
           :on-change (lambda (v) (vui-set-state :password v)))
#+end_src

** vui-field-value

#+begin_src elisp
(vui-field-value KEY) -> string or nil
#+end_src

Get current value of field with KEY, without triggering re-render.

#+begin_src elisp
(vui-field :key 'my-input :size 20)
(vui-button "Submit"
            :on-click (lambda ()
                        (process (vui-field-value 'my-input))))
#+end_src

** vui-checkbox

#+begin_src elisp
(vui-checkbox &rest PROPS) -> vnode
#+end_src

Create a checkbox.

| Property     | Type     | Description                    |
|--------------+----------+--------------------------------|
| =:checked=   | boolean  | Whether checked                |
| =:on-change= | function | Called with boolean on toggle  |
| =:label=     | string   | Label after checkbox           |
| =:key=       | any      | Reconciliation key             |

#+begin_src elisp
(vui-checkbox :checked enabled
              :label "Enable feature"
              :on-change (lambda (v) (vui-set-state :enabled v)))
#+end_src

** vui-select

#+begin_src elisp
(vui-select &rest ARGS) -> vnode
#+end_src

Create a selection button (minibuffer-based selection).

| Property     | Type      | Description                              |
|--------------+-----------+------------------------------------------|
| =:value=     | any       | Current selection (an option's value)    |
| =:options=   | list      | Strings, or =(value . label)= cons cells |
| =:on-change= | function  | Called with the selected option's value  |
| =:prompt=    | string    | Minibuffer prompt (default "Select: ")   |
| =:key=       | any       | Reconciliation key                       |

For =(value . label)= options, the button text and completion candidates
show the labels, while =:on-change= receives the corresponding value -
which may be any Lisp object (symbol, number, string). Labels should be
unique; otherwise the first matching option wins. Plain string options
are their own label.

#+begin_src elisp
;; Simple list
(vui-select :value current-theme
            :options '("light" "dark" "system")
            :on-change (lambda (v) (vui-set-state :theme v)))

;; Values with display labels - on-change receives 'active etc.
(vui-select :value status
            :options '((active . "Active")
                       (pending . "Pending")
                       (closed . "Closed"))
            :on-change (lambda (v) (vui-set-state :status v)))
#+end_src

* Layout

** vui-hstack

#+begin_src elisp
(vui-hstack &rest ARGS) -> vnode
#+end_src

Horizontal layout. ARGS can start with options, then children.

| Option     | Type    | Default | Description                                 |
|------------+---------+---------+---------------------------------------------|
| =:spacing= | integer | 1       | Spaces between children                     |
| =:face=    | face    | nil     | Applied beneath children's faces (see =vui-region=) |
| =:keymap=  | keymap  | nil     | Cascades beneath children's keymaps (see =vui-region=) |
| =:key=     | any     | nil     | Reconciliation key                          |

#+begin_src elisp
(vui-hstack :spacing 2
            (vui-text "Label:")
            (vui-field :size 10))

;; Highlight a whole row, separators included
(vui-hstack :face 'highlight
            (vui-text "item")
            (vui-button "open" :on-click #'my-open))
#+end_src

** vui-vstack

#+begin_src elisp
(vui-vstack &rest ARGS) -> vnode
#+end_src

Vertical layout. ARGS can start with options, then children.

| Option     | Type    | Default | Description                                 |
|------------+---------+---------+---------------------------------------------|
| =:spacing= | integer | 0       | Blank lines between children                |
| =:indent=  | integer | 0       | Left indent in spaces                       |
| =:face=    | face    | nil     | Applied beneath children's faces (see =vui-region=) |
| =:keymap=  | keymap  | nil     | Cascades beneath children's keymaps (see =vui-region=) |
| =:key=     | any     | nil     | Reconciliation key                          |

#+begin_src elisp
(vui-vstack :spacing 1 :indent 2
            (vui-text "Item 1")
            (vui-text "Item 2"))
#+end_src

** vui-box

#+begin_src elisp
(vui-box CHILD &rest PROPS) -> vnode
#+end_src

Fixed-width container for CHILD.

| Property         | Type    | Default | Description                                 |
|------------------+---------+---------+---------------------------------------------|
| =:width=         | integer | 20      | Width in characters                         |
| =:align=         | keyword | :left   | :left, :center, :right                      |
| =:padding-left=  | integer | 0       | Left padding                                |
| =:padding-right= | integer | 0       | Right padding                               |
| =:face=          | face    | nil     | Applied beneath child's faces (see =vui-region=) |
| =:keymap=        | keymap  | nil     | Cascades beneath child's keymaps (see =vui-region=) |
| =:key=           | any     | nil     | Reconciliation key                          |

#+begin_src elisp
(vui-box (vui-text "Centered") :width 30 :align :center)
#+end_src

** vui-flex

#+begin_src elisp
(vui-flex [OPTIONS...] CHILDREN...) -> vnode
(vui-flex-item [OPTIONS...] CHILD) -> vnode
#+end_src

Horizontal row that distributes a total width among its children.

| Option     | Type            | Default       | Description                          |
|------------+-----------------+---------------+--------------------------------------|
| =:width=   | number/fn/symbol | =fill-column= | Total width; also accepts =window= or a function resolved at render time |
| =:spacing= | integer         | 1             | Spaces between children              |
| =:justify= | keyword         | :start        | :start, :center, :end, :space-between (when nothing grows; not applied under =:wrap=) |
| =:wrap=    | boolean         | nil           | Children wrap into rows in source order when they stop fitting |
| =:face=    | face            | nil           | Applied beneath children's faces     |
| =:keymap=  | keymap          | nil           | Cascades beneath children's keymaps  |
| =:key=     | any             | nil           | Reconciliation key                   |

Children render at their natural width. Wrap a child in =vui-flex-item=
to give it a proportional share of the leftover width instead:

| Option       | Type | Default | Description                                 |
|--------------+------+---------+---------------------------------------------|
| =:grow=      | int  | 1       | Proportional weight among growing children  |
| =:min-width= | int  | nil     | Under =:wrap=, the minimum width the child occupies (chars). A function child may shrink down to it (it is also its width during row partitioning); a vnode child never shrinks, so it only raises the width |

A =vui-flex-item= child is either a vnode (rendered inside a box padded
to the allotted width) or a function called with the allotted width -
which is what lets fields actually fill remaining space:

#+begin_src elisp
(vui-flex :width 'fill-column
  (vui-text "Name:")
  (vui-flex-item :grow 1
    (lambda (width) (vui-field :size width :key 'name)))
  (vui-button "Save"))
#+end_src

Flex rows subtract inherited vstack indentation from their available
width. When the children's natural widths already exceed the total,
everything renders at natural width and growers get zero. Without
=:wrap=, rows are single-line by design: multi-line children are
measured by their widest line, but the layout does not account for
their extra lines.

With =:wrap=, children fill a row while they fit (at their minimum
widths) and continue on the next row, in source order. Each row
distributes width on its own: growers grow into their row's leftover,
and a function child in a =vui-flex-item= with =:min-width= may shrink
down to it when the row runs out of space. Vnode children render at
one width and never shrink, so a row packs them at their content
width; their =:min-width= only raises it. A row whose children are all single-line
renders them in place, exactly like a non-wrapped flex - components
and widgets keep their identity. A row containing a multi-line child
(a box, a vstack, a table) is composed from the children's rendered
text so the blocks sit side by side. Composed rows are content:
buttons keep working, but components in them never mount - no state,
no lifecycle, and =vui-set-state= from inside them does nothing - and
widget fields do not survive composition. Keep stateful components on
single-line rows (or outside =:wrap=), and give components a =:key=
so measurement matches them exactly.

For live resizing, pass =window= or a function as =:width=; mounted
buffers reflow on window size changes automatically (see
=vui-rerender-on-resize=).

** vui-grid

#+begin_src elisp
(vui-grid [OPTIONS...] CELLS...) -> vnode
#+end_src

Responsive grid of equal-width tracks. Cells fill rows in source order
(the final row may be incomplete); the column count falls with the
available width.

| Option               | Type            | Default       | Description                          |
|----------------------+-----------------+---------------+--------------------------------------|
| =:columns=           | integer         | 2             | Column count to aim for              |
| =:min-column-width=  | integer         | nil           | Smallest acceptable track (chars); columns drop until tracks fit, never below one |
| =:width=             | number/fn/symbol | =fill-column= | Total width, like =vui-flex= =:width= |
| =:spacing=           | integer         | 1             | Gap between tracks                   |
| =:row-spacing=       | integer         | 0             | Blank lines between rows             |
| =:face=              | face            | nil           | Applied beneath cells' faces         |
| =:keymap=            | keymap          | nil           | Cascades beneath cells' keymaps      |
| =:key=               | any             | nil           | Reconciliation key                   |

Tracks are equal integer widths, remainders on the earlier tracks. A
cell wider than its track widens that whole column, across every row,
so columns stay aligned. A cell may be a function: it is called with
the track width in characters and returns a vnode. Identity follows
=vui-flex= =:wrap=: single-line rows render in place, rows containing
a multi-line cell are composed as text (buttons work; components in
composed rows never mount, and widget fields do not survive
composition). Keep stateful components in single-line cells, and give
components a =:key= so measurement matches them exactly. An empty
cell keeps its track, so columns stay aligned.

#+begin_src elisp
(vui-grid :width 'window :columns 3 :min-column-width 24
  (panel-1) (panel-2) (panel-3))
#+end_src

** vui-rerender-on-resize

#+begin_src elisp
(vui-rerender-on-resize &optional BUFFER)
(vui-cancel-rerender-on-resize &optional BUFFER)
#+end_src

Re-render BUFFER's VUI instances (the =vui-mount= root and any inline
instances) whenever its window size changes. BUFFER defaults to the
current buffer. Re-renders go through the normal deferred scheduling,
so a burst of resize events coalesces into one re-render per instance.
The hook is buffer-local and dies with the buffer;
=vui-cancel-rerender-on-resize= removes it earlier.

=vui-mount= and =vui-mount-inline= install the hook automatically, so
window-width-dependent layouts (=vui-flex= and =vui-grid= with
=:width 'window= or a function) reflow with no setup. Set the
defcustom =vui-rerender-on-resize-default= to nil for the previous
opt-in behavior; buffers rendered with plain =vui-render= still need
the explicit call.

** vui-region

#+begin_src elisp
(vui-region [OPTIONS...] CHILDREN...) -> vnode
#+end_src

Styling wrapper: apply a face and/or keymap to the children's whole
extent, without affecting layout.

| Property  | Type   | Description                                  |
|-----------+--------+----------------------------------------------|
| =:face=   | face   | Applied beneath the children's own faces     |
| =:keymap= | keymap | Cascades beneath the children's own keymaps  |
| =:key=    | any    | Reconciliation key                           |

The face is layered with the lowest priority: children that set their
own faces keep them, while plain text - including separators, padding,
and indentation inserted by nested layout containers - gets the region
face. Useful for row highlighting and panel backgrounds.

The keymap cascades innermost-first: a nested region's or a button's
own keymap wins for keys it defines, unbound keys fall through to the
region's map, then to the buffer's usual keymaps. Buttons keep their
default =RET= / =TAB= behavior, and input fields keep their own keymap.

#+begin_src elisp
;; Highlight a selected row and give it row-level bindings
(vui-region :face 'highlight :keymap my-row-map
  (vui-hstack
   (vui-text "item")
   (vui-button "open" :on-click #'my-open)))
#+end_src

** vui-table

#+begin_src elisp
(vui-table &rest ARGS) -> vnode
#+end_src

Create a table layout. Cell contents can be strings or vnodes (including
interactive widgets like buttons, fields, and components).

| Property          | Type    | Description                                    |
|-------------------+---------+------------------------------------------------|
| =:columns=        | list    | List of column specs (see below)               |
| =:rows=           | list    | List of rows, each a list of cells             |
| =:border=         | symbol  | nil, :ascii, or :unicode                       |
| =:sticky-header=  | boolean | Render the header row into =header-line-format= (see below) |
| =:header-face=    | face    | Face for header cells (default =vui-table-header=) |
| =:border-face=    | face    | Face for border characters (default =vui-table-border=) |
| =:key=            | any     | Reconciliation key                             |

Headers render in the =vui-table-header= face (inherits =bold=) and
border characters in =vui-table-border= (no attributes by default), so
both can be themed globally via =M-x customize-face= or overridden per
table with the properties above.

When =:border= is set, cells are automatically padded with 1 space on each
side for readability (e.g., =| value |= instead of =|value|=).

*** Column Spec Properties

| Property     | Type    | Description                             |
|--------------+---------+-----------------------------------------|
| =:header=    | string  | Header text                             |
| =:width=     | integer | Target width for cell content           |
| =:min-width= | integer | Minimum width, expand as needed         |
| =:grow=      | boolean | Pad short content, expand for long      |
| =:truncate=  | boolean | Truncate long content with "..."        |
| =:align=     | keyword | :left (default), :center, :right        |

*** Width Behavior Table

| =:width= | =:grow= | =:truncate= | Content vs Width | Result                                    |
|----------+---------+-------------+------------------+-------------------------------------------|
| W        | nil     | nil         | content < W      | Column shrinks to content size            |
| W        | nil     | nil         | content > W      | Overflow with broken bar (¦)              |
| W        | t       | nil         | content < W      | Column = W, content padded                |
| W        | t       | nil         | content > W      | Column expands to fit content             |
| W        | nil     | t           | content < W      | Column shrinks to content size            |
| W        | nil     | t           | content > W      | Column = W, content truncated with "..."  |
| W        | t       | t           | content < W      | Column = W, content padded                |
| W        | t       | t           | content > W      | Column = W, content truncated with "..."  |
| nil      | -       | -           | any              | Column auto-sizes to content              |

*** Sticky Header

With =:sticky-header t= the header row stays visible while a long
table scrolls. The table renders into the buffer exactly as without
it - header row included - and a copy of the header row is pinned in
the window's =header-line-format= *only while the window is scrolled
into the table's body*, that is, while the in-buffer header row is
above the window start but the table has not ended yet:

#+begin_src elisp
(vui-table
 :sticky-header t
 :columns '((:header "Name" :min-width 15)
            (:header "Age" :width 5 :align :right))
 :rows lots-of-rows
 :border :ascii)
#+end_src

This is the sticky-scroll model rather than the =tabulated-list-mode=
model: content before and after the table (titles, filter fields,
footers) behaves normally, windows showing that content pin nothing,
and a buffer can hold several sticky tables - as the window scrolls
from one into the next, the pinned header hands over.

The pinned copy is read live from the buffer at redisplay time, so it
always matches the current column widths, borders, padding, faces,
and even surrounding indentation - columns line up exactly, and
re-renders that change widths are picked up automatically. Evaluation
is per window: two windows on the same buffer scrolled to different
tables each pin their own header. Unmounting the tree (or mounting
another one over the buffer) restores whatever =header-line-format=
was there before.

Faces: the pinned copy keeps =vui-table-header= (or =:header-face=)
and =vui-table-border= (or =:border-face=), drawn on top of the
window's =header-line= face, which typically adds its own background.
The defaults (bold, no colors) compose well with it; if you override
the faces with explicit colors, check them against the =header-line=
background.

Limitations:

- Headers must be plain strings. The pinned copy lives in the header
  line, outside the buffer, so vui navigation (=vui-forward= /
  =vui-backward=) and buttons cannot reach it; a vnode header signals
  an error when =:sticky-header= is on. (The in-buffer header row is
  plain text anyway, so nothing is lost.)
- While any sticky table is mounted, the header line is reserved: it
  shows an empty line when no table is pinned, rather than appearing
  and disappearing as you scroll (which would make the window content
  jump by a line).

See =docs/examples/16-sticky-table.el= for a runnable demo: a long
filterable table with widgets before and after it.

#+begin_src elisp
(vui-table
 :columns '((:header "Name" :min-width 15)
            (:header "Age" :width 5 :align :right))
 :rows '(("Alice" "30")
         ("Bob" "25"))
 :border :ascii)
#+end_src

*** Interactive Cells

Cells can contain any vnode, including buttons and components:

#+begin_src elisp
(vui-table
 :columns '((:header "Item" :width 20)
            (:header "Action" :width 10))
 :rows `(("Apple" ,(vui-button "[Buy]" :on-click (lambda () (message "Bought!"))))
         ("Banana" ,(vui-button "[Buy]" :on-click (lambda () (message "Bought!")))))
 :border :ascii)
#+end_src

For interactive tables, embed components with callbacks:

#+begin_src elisp
(vui-table
 :columns '((:header "Name" :width 15)
            (:header "Score" :width 8))
 :rows (mapcar (lambda (item)
                 (list (plist-get item :name)
                       (vui-component 'score-input
                                      :key (plist-get item :id)
                                      :value (plist-get item :score)
                                      :on-change on-score-change)))
               items)
 :border :ascii)
#+end_src

*** Dynamic Column Widths

Calculate column widths at render time based on content:

#+begin_src elisp
(let* ((max-len (apply #'max (mapcar #'length names)))
       (col-width (max 18 (min 48 (+ max-len 2)))))
  (vui-table
   :columns `((:header "Name" :width ,col-width)
              (:header "Value" :width 10))
   :rows ...))
#+end_src

See =docs/examples/05-wine-tasting.el= for a complete example demonstrating
interactive buttons, dynamic column widths, and real-time computed statistics.

** vui-list

#+begin_src elisp
(vui-list ITEMS RENDER-FN [KEY-FN] &key VERTICAL INDENT SPACING) -> vnode
#+end_src

Render a list of items with proper reconciliation.

- ITEMS :: List of items to render
- RENDER-FN :: =(lambda (item) vnode)= - renders each item
- KEY-FN :: =(lambda (item) key)= - extracts key (default: identity). Optional;
  a keyword in this position starts the options instead, so passing an
  explicit nil before keyword options is not required.
- =:vertical= :: If non-nil (default t), returns a =vstack=; otherwise returns an =hstack=
- =:indent= :: Left indentation in spaces (default 0)
- =:spacing= :: Blank lines between items for vertical (default 0), spaces for horizontal (default 1)
- =:face= :: Applied beneath the items' own faces (see =vui-region=)
- =:keymap= :: Cascades beneath the items' own keymaps (see =vui-region=)

Returns a =vstack= for vertical lists, =hstack= for horizontal lists. This ensures
proper indent propagation when nested inside other layout containers.

#+begin_src elisp
;; Vertical list (default) - each item on its own line
(vui-list todos
          (lambda (todo)
            (vui-text (plist-get todo :text)))
          (lambda (todo)
            (plist-get todo :id)))

;; With indentation
(vui-list items #'vui-text :indent 2)

;; Horizontal list - items on same line with spacing
(vui-list tags
          (lambda (tag) (vui-text (format "[%s]" tag)))
          :vertical nil)
#+end_src

* Components

** vui-component

#+begin_src elisp
(vui-component TYPE &rest PROPS-AND-CHILDREN) -> vnode
#+end_src

Create a component vnode.

- TYPE :: Symbol naming a defined component
- PROPS-AND-CHILDREN :: Plist of props, optionally ending with =:children=

#+begin_src elisp
(vui-component 'greeting :name "Alice")
(vui-component 'card :title "Hello" :children (list child1 child2))
#+end_src

* State Management

** vui-set-state

#+begin_src elisp
(vui-set-state KEY VALUE)
#+end_src

Set state KEY to VALUE and schedule re-render.

Must be called from within a component context (event handler, lifecycle hook).

If VALUE is a function, it is called with the current value and the result
is used as the new value. This is essential for async callbacks where
captured variables may be stale.

#+begin_src elisp
;; Direct value
(vui-button "Increment"
            :on-click (lambda ()
                        (vui-set-state :count (1+ count))))

;; Functional update (for async callbacks)
(run-with-timer 1 1
  (vui-with-async-context
    (vui-set-state :count #'1+)))  ; Gets current value, returns incremented

;; Lambda for complex updates
(vui-set-state :items (lambda (old) (cons new-item old)))
#+end_src

** vui-batch

#+begin_src elisp
(vui-batch &rest BODY)
#+end_src

Batch multiple state updates into a single re-render.

#+begin_src elisp
(vui-batch
 (vui-set-state :name "Bob")
 (vui-set-state :age 30)
 (vui-set-state :active t))
;; Single re-render instead of three
#+end_src

** vui-flush-sync

#+begin_src elisp
(vui-flush-sync)
#+end_src

Force immediate re-render, bypassing any pending deferred timers.

** vui-with-async-context

#+begin_src elisp
(vui-with-async-context BODY...) -> function
#+end_src

Capture component context for use in async callbacks. Returns a function that,
when called, restores the context and executes BODY.

Use this when you need =vui-set-state= in:
- Timer callbacks (=run-with-timer=, =run-with-idle-timer=)
- Emacs hook functions
- Process sentinels (when not using =vui-use-async=)
- Any callback that runs asynchronously

#+begin_src elisp
;; Timer example - use functional update for state-based values
(vui-use-effect ()
  (let ((timer (run-with-timer 1 1
                 (vui-with-async-context
                   (vui-set-state :seconds #'1+)))))
    (lambda () (cancel-timer timer))))

;; Hook example - (frame-width) is called fresh, no capture issue
(vui-use-effect ()
  (let ((handler (vui-with-async-context
                   (vui-set-state :width (frame-width)))))
    (add-hook 'window-size-change-functions handler)
    (lambda ()
      (remove-hook 'window-size-change-functions handler))))
#+end_src

*Note:* You do NOT need this for widget callbacks (buttons, fields) or
=vui-use-async= loaders - those handle context automatically.

** vui-async-callback

#+begin_src elisp
(vui-async-callback (ARGS...) BODY...) -> function
#+end_src

Create an async callback that captures component context and accepts arguments.
Like =vui-with-async-context=, but the returned function accepts ARGS which are
bound when executing BODY.

Use this when an async operation passes data to your callback:
- API responses
- Process output
- Any async operation that returns a value

#+begin_src elisp
;; Fetch data and set state with the result
(vui-use-effect ()
  (fetch-data-async
    (vui-async-callback (result)
      (vui-set-state :data result))))

;; Multiple arguments
(vui-use-effect ()
  (my-api-call
    (vui-async-callback (data status)
      (vui-set-state :data data)
      (vui-set-state :status status))))
#+end_src

*Compare with =vui-with-async-context=:*
- =vui-with-async-context= - fire-and-forget callbacks (timers, hooks)
- =vui-async-callback= - callbacks that receive data from async operations

* Hooks

** use-effect

#+begin_src elisp
(vui-use-effect (DEPS...) BODY...)
#+end_src

Run side effect when DEPS change.

- Runs after first render
- Runs after re-render if any dep changed
- If BODY returns a function, it's called as cleanup

#+begin_src elisp
;; Run once on mount
(vui-use-effect ()
  (message "Mounted"))

;; Run when count changes
(vui-use-effect (count)
  (message "Count: %d" count))

;; With cleanup
(vui-use-effect (user-id)
  (let ((timer (run-with-timer 1 nil #'refresh)))
    (lambda () (cancel-timer timer))))
#+end_src

** use-ref

#+begin_src elisp
(vui-use-ref INITIAL-VALUE) -> (VALUE . nil)
#+end_src

Create mutable ref that persists across renders.

Access via =(car ref)=, set via =(setcar ref new-value)=.

Modifying a ref does NOT trigger re-render.

#+begin_src elisp
(let ((timer-ref (vui-use-ref nil)))
  (vui-use-effect ()
    (setcar timer-ref (run-with-timer 1 1 #'tick))
    (lambda () (cancel-timer (car timer-ref)))))
#+end_src

** use-callback

#+begin_src elisp
(vui-use-callback (DEPS...) BODY)
#+end_src

Create memoized callback that stays stable when DEPS unchanged.

#+begin_src elisp
(let ((handle-click (vui-use-callback (item-id)
                      (lambda () (delete-item item-id)))))
  (vui-button "Delete" :on-click handle-click))
#+end_src

** use-callback*

#+begin_src elisp
(vui-use-callback* (DEPS...) :compare COMPARE BODY)
#+end_src

Like =vui-use-callback= with configurable comparison.

COMPARE options:
- =eq= - identity comparison (fast)
- =equal= - structural comparison (default); strings must also match in
  text properties
- function - custom =(lambda (old-deps new-deps) bool)=

** use-memo

#+begin_src elisp
(vui-use-memo (DEPS...) BODY)
#+end_src

Cache computed value, recompute only when DEPS change.

#+begin_src elisp
(let ((filtered (vui-use-memo (items filter)
                  (seq-filter (lambda (i)
                                (string-match-p filter (plist-get i :name)))
                              items))))
  (vui-list filtered #'render-item))
#+end_src

** use-memo*

#+begin_src elisp
(vui-use-memo* (DEPS...) :compare COMPARE BODY)
#+end_src

Like =vui-use-memo= with configurable comparison.

** use-async

#+begin_src elisp
(vui-use-async KEY LOADER) -> plist
#+end_src

Asynchronously load data using LOADER, identified by KEY.

- KEY :: Cache key; when it changes, a new load is triggered
- LOADER :: =(lambda (resolve reject) ...)= - call =resolve= with data or =reject= with error

Returns a plist:

| Key       | Description                            |
|-----------+----------------------------------------|
| =:status= | One of: =pending=, =ready=, or =error= |
| =:data=   | The loaded data (when =ready=)         |
| =:error=  | Error message (when =error=)           |

#+begin_src elisp
(let ((result (vui-use-async user-id
                (lambda (resolve _reject)
                  (funcall resolve (fetch-user user-id))))))
  (pcase (plist-get result :status)
    ('pending (vui-text "Loading..."))
    ('error (vui-text (format "Error: %s" (plist-get result :error))))
    ('ready (vui-text (format "User: %s" (plist-get result :data))))))
#+end_src

* Context

** defcontext

#+begin_src elisp
(vui-defcontext NAME &optional DEFAULT-VALUE DOCSTRING)
#+end_src

Define a context.

Creates:
- =NAME-context= - The context object
- =NAME-provider= - Function to provide value
- =use-NAME= - Convenience hook to consume value

#+begin_src elisp
(vui-defcontext theme 'light "Current UI theme.")

;; Provide
(theme-provider 'dark
  (vui-component 'my-app))

;; Consume (in component render)
(let ((theme (vui-use-context theme-context)))
  (vui-text (format "Theme: %s" theme)))

;; Equivalent, via the generated hook
(let ((theme (use-theme)))
  (vui-text (format "Theme: %s" theme)))
#+end_src

The =NAME=-derived symbols follow whatever prefix NAME carries, so
packages should use a prefixed NAME (e.g. =my-pkg-theme=) and prefer
=vui-use-context= over the unprefixed =use-NAME= hook.

** vui-use-context

#+begin_src elisp
(vui-use-context CONTEXT) -> value
#+end_src

Return the current value of CONTEXT (a =NAME-context= object created by
=vui-defcontext=). Searches the providers in scope at the current render
position; returns the context's default value when no provider is found.
Must be called during render.

* Error Handling

** vui-error-boundary

#+begin_src elisp
(vui-error-boundary &key FALLBACK ON-ERROR ID CHILDREN) -> vnode
#+end_src

Catch render errors in CHILDREN and display FALLBACK.

| Property   | Type     | Description                                |
|------------+----------+--------------------------------------------|
| =:fallback= | function | =(lambda (error) vnode)= rendered on error |
| =:on-error= | function | Called once with error when first caught   |
| =:id=       | any      | Optional identifier (default: tree position) |
| =:children= | list     | Protected content                          |

When =:id= is omitted, the boundary's identity is derived from its
position in the rendered tree (stable across re-renders as long as the
tree shape around it does not change). Error state is scoped to the
mounted tree, so boundaries in different buffers never interfere and a
fresh mount starts clean.

#+begin_src elisp
(vui-error-boundary
 :id 'main-boundary
 :fallback (lambda (_err) (vui-text "Something went wrong!" :face 'error))
 :on-error (lambda (err) (log-error err))
 :children (list (vui-component 'risky-component)))
#+end_src

** vui-reset-error-boundary

#+begin_src elisp
(vui-reset-error-boundary &optional ID)
#+end_src

Reset error state for boundary ID, allowing retry. When ID is nil,
resets every boundary in the current tree. Call it from a component
callback or with the VUI buffer current.

* Configuration Variables

** Error Handling

| Variable                    | Default | Description                         |
|-----------------------------+---------+-------------------------------------|
| =vui-lifecycle-error-handler= | warn   | Handler for lifecycle errors        |
| =vui-event-error-handler=     | warn   | Handler for event errors            |
| =vui-last-error=              | nil    | Last error: (TYPE ERROR CONTEXT)    |

Handler values:
- =warn= - Display warning (default)
- =message= - Display in echo area
- =signal= - Re-signal error
- =ignore= - Silently ignore
- function - Custom =(lambda (hook-name err instance) ...)=

** Layout

| Variable           | Default | Description                                                     |
|--------------------+---------+-----------------------------------------------------------------|
| =vui-width-mode=   | char    | How layout measures and pads text: =char= (=string-width=, tty-faithful) or =pixel= (=string-pixel-width=, aligns mixed fonts, emoji, CJK). Applies to every layout primitive at once; declared widths stay in characters. See the layout guide. |

** Performance

| Variable               | Default | Description                                 |
|------------------------+---------+---------------------------------------------|
| =vui-render-delay=     | 0.01    | Seconds before deferred render (nil=immediate) |
| =vui-timing-enabled=    | nil    | Enable timing collection           |

** Debugging

| Variable               | Default                                       | Description              |
|------------------------+-----------------------------------------------+--------------------------|
| =vui-debug-enabled=    | nil                                           | Enable debug logging     |
| =vui-debug-log-phases= | (render mount update unmount state-change)    | Phases to log            |

* Developer Tools

** vui-inspect

#+begin_src elisp
(vui-inspect &optional INSTANCE)
#+end_src

Display component tree in inspector buffer.

Shows hierarchical view with props and state for each component.

** vui-inspect-state

#+begin_src elisp
(vui-inspect-state &optional INSTANCE)
#+end_src

Display only components with state.

** vui-get-instance-by-id

#+begin_src elisp
(vui-get-instance-by-id ID &optional INSTANCE) -> instance or nil
#+end_src

Find component instance by ID.

** vui-get-component-instances

#+begin_src elisp
(vui-get-component-instances COMPONENT-TYPE &optional INSTANCE) -> list
#+end_src

Find all instances of COMPONENT-TYPE.

** vui-report-timing

#+begin_src elisp
(vui-report-timing &optional LAST-N)
#+end_src

Display timing report in buffer.

Groups by component, shows time per phase.

** vui-get-timing

#+begin_src elisp
(vui-get-timing) -> list
#+end_src

Return raw timing data.

Each entry: =(:phase PHASE :component NAME :duration SECONDS :timestamp TIME)=

** vui-clear-timing

#+begin_src elisp
(vui-clear-timing)
#+end_src

Clear all timing data.

** vui-debug-show

#+begin_src elisp
(vui-debug-show)
#+end_src

Show the =*vui-debug*= buffer.

** vui-debug-clear

#+begin_src elisp
(vui-debug-clear)
#+end_src

Clear debug log buffer.

* Quick Reference

** Creating Components

#+begin_src elisp
(vui-defcomponent NAME (PROPS...)
  :state ((VAR INIT) ...)
  :on-mount FORM
  :on-update FORM
  :on-unmount FORM
  :should-update FORM
  :memo BOOL
  :render VNODE)
#+end_src

** Primitives

| Function        | Description           |
|-----------------+-----------------------|
| =vui-text=      | Text content          |
| =vui-newline=   | Line break            |
| =vui-space=     | Horizontal space      |
| =vui-fragment=  | Group without wrapper |
| =vui-button=    | Clickable button      |
| =vui-field=     | Text input            |
| =vui-checkbox=  | Toggle checkbox       |
| =vui-select=    | Selection dropdown    |

** Layout

| Function     | Description        |
|--------------+--------------------|
| =vui-hstack= | Horizontal layout  |
| =vui-vstack= | Vertical layout    |
| =vui-box=    | Fixed-width box    |
| =vui-table=  | Table layout       |
| =vui-list=   | List with keys     |

** Hooks

| Hook           | Description               |
|----------------+---------------------------|
| =vui-use-effect=   | Side effects              |
| =vui-use-ref=      | Mutable reference         |
| =vui-use-callback= | Memoized callback         |
| =vui-use-memo=     | Memoized value            |
| =vui-use-async=    | Async data loading        |

** State

| Function         | Description                |
|------------------+----------------------------|
| =vui-set-state=  | Update component state     |
| =vui-batch=      | Batch multiple updates     |
| =vui-flush-sync= | Force immediate render     |

** Context

| Macro/Function   | Description          |
|------------------+----------------------|
| =vui-defcontext=     | Define context       |
| =NAME-provider=  | Provide value        |
| =use-NAME=       | Consume value        |

** Debugging

| Function              | Description                |
|-----------------------+----------------------------|
| =vui-inspect=         | Show component tree        |
| =vui-inspect-state=   | Show state only            |
| =vui-report-timing=   | Performance report         |
| =vui-debug-show=      | Show debug log             |
