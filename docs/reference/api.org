...
| =:min-width= | int  | nil     | Under =:wrap=, floor the child may shrink to (chars); for a function child also its width during row partitioning |
...
and a =vui-flex-item= with =:min-width= may shrink down to it when the
row runs out of space. A row whose children are all single-line
